...
describe Person do
  ...
  describe 'can create an Account' do
    ...
    it 'of Account class ' do
      expect(subject.account).to be_an_instance_of Account # whatever object we are looking at
    end
  ...
  describe 'can manage funds if an account has been created' do
    ...
    it 'withdraw is expected to raise error if no ATM is passed in' do
      command = lambda { subject.get_cash_from_atm(amount: 100, pin: subject.account.pin_code, account: subject.account) }
    ...
    it 'funds are added to cash - deducted from account balance' do
      subject.cash = 100
      subject.deposit(100)
      subject.get_cash_from_atm(amount: 100, pin: subject.account.pin_code, account: subject.account, atm: atm)
